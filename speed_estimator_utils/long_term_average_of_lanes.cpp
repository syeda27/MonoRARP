...
void Lane_detector::long_term_average_of_lanes(int average_window, double &muy_lane_ave2, double &mux_lane_ave2, double &base_ptx_ave2, double &base_pty_ave2, double &muy_lane_ave, double &mux_lane_ave, double &base_ptx_ave, double &base_pty_ave, int &count_lanes_average_vec2_out,int &count_lanes_average_vec2_out2)
{
    ...
    if (count_lanes_average_vec>=average_window)
    {
        ...
        line(img6, Point(int(base_ptx_ave), int(base_pty_ave)), Point(int(x1_lane),int(0)), Scalar(255,255,255), 1, 1);
        line(img6, Point(int(base_ptx_ave), int(base_pty_ave)), Point(int(x2_lane),int(H)), Scalar(255,255,255), 1, 1);   
    ...
    if (count_lanes_average_vec2>=average_window)
    {
        ...
        line(img6, Point(int(base_ptx_ave2), int(base_pty_ave2)), Point(int(x1_lane),int(0)), Scalar(255,255,255), 1, 1);
        line(img6, Point(int(base_ptx_ave2), int(base_pty_ave2)), Point(int(x2_lane),int(H)), Scalar(255,255,255), 1, 1);
    }  
