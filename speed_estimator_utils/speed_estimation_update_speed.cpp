...
void Lane_detector::speed_estimator_update(Mat image_color, double time)
{
    ...
    double scanning_window_width=200;//220;//200;
    ...
    double horizontal_tolerance = 30;//70;//180;
    ...
    int Right_margin_detection = 1900;
    ...
    //for (int image_number=2250; image_number<2550 ;image_number++)
    //for (int image_number=3200; image_number<3500 ;image_number++)
    //{

        //file_name = "/media/juan/3036-3031/GH090041_frames/frames_flipped/"+to_string(image_number)+".jpeg.jpg";
        //cout<<file_name<<endl;
        //Lane_detector::image_reading(file_name);
            ...
            for (int yregion=scan_y_ini; yregion<=scan_y_end; yregion += scan_y_step)
            {
                ...
                if (count_angles_per_region>=2)
                {
                    ...
                    if (criteria_flag == 1)
                    {
                        ...
                        aligned_to_tracked_lane_flag = Lane_detector::determination_of_parallelism();
