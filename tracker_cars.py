...
initialize_vehicles=np.zeros(10)

x_particle=np.zeros(10)
y_particle=np.zeros(10)
x_particle2=np.zeros(10)
y_particle2=np.zeros(10)
x_particle3=np.zeros(10)
y_particle3=np.zeros(10)

x_particle_vehicles=np.zeros((10,10))
y_particle_vehicles=np.zeros((10,10))


distance_to_particle_identified_previous=np.zeros(10)
distance_to_particle_identified_previous2=np.zeros(10)
distance_to_particle_identified_previous3=np.zeros(10)

distance_to_particle_identified_previous_vehicles=np.zeros((10,10))
...
delta_x_vehicles=np.zeros(10)
delta_y_vehicles=np.zeros(10)


count_holding_vehicles=np.zeros(10)

count_holding=0
count_holding2=0
count_holding3=0
initialization_delta=0
holding_tracked=np.zeros(10)
...
cx_previous_vehicles=np.zeros(10)
cy_previous_vehicles=np.zeros(10)
            ...
            for k2 in range(0,Lc):
                ...
                for k3 in range(1,count_tracked_vehicles+1):
                    ...
                    if ((cx-cx_tracked[k3])**2+(cy-cy_tracked[k3])**2)**0.5<50:
                        ...
                        if count_holding_vehicles[k3]>0 or cy_tracked[k3]>1600: #if the vehicle being probed is holding then we need to be more restrictive because the holding is being done with cx_previous and cy_previous which is farther from current bounding box that in principle corresponds to holding vehicle
                            if ((cx-cx_tracked[k3])**2+(cy-cy_tracked[k3])**2)**0.5<100:
                                conflict=1
                                break
