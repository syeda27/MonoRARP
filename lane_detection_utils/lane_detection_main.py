...
for image_number in range(1085, 2665):
    ...
    for xregion in range(scan_x_ini, scan_x_end, scan_x_step):
        for yregion in range(scan_y_ini, scan_y_end, scan_y_step):
            ...
            rx1, rx2, ry1, ry2, angles, count_angles_per_region = ⁠\
                scan_region(dlines,
                            xregion,
                            yregion,
            ...
            if count_angles_per_region> = 2:  #perform all the following processing only if 2 or more line-segments exists per scanning region
                ...
                if ry2[top_left] > ry1[top_right] and ⁠\
                        ry2[top_right] > ry1[top_left] and \
                        abs(angles[top_right] - angles[top_left]) < 6:
                    ...
                    whitemarkings_average, removed3, removed4) = \
                    outlayer_removal_and_average_brightness_computation(
                        road1_vec,
                        road2_vec,
                        road3_vec,
                        road4_vec,
                    ...
                    base_ptx_lane_vec, base_pty_lane_vec, count_lanes) = \
                        lane_signature_detection(
                            road1_average,
                            road2_average,
                            road3_average,
                            road4_average,
