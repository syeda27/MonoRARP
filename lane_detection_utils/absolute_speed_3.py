...
# 0.7*h1 where h1 is the height of the image subframe used to perform Lane Detection and speed estimation. The procedure below uses as input the unit vector of the line representing the Lane  
...
def absolute_speed_estimation(muy_lane_vec_speed,mux_lane_vec_speed,base_ptx_lane_vec_speed,base_pty_lane_vec_speed,h1,capture_frameindex_for_speed,frameindex_for_speed_previous,frameindex_for_speed,image_number,white_mark_hit,count_scanned_lines_reverse_for_speed_previous,count_scanned_lines_reverse_for_speed,count_scanned_lines_for_speed_previous,count_scanned_lines_for_speed,offset_adjustment,range_adjustment_left,range_adjustment_right,img6,img2):
    ...
    cv2.line(img6, (int(x2_lane-50), int(0.7*h1)), (int(x2_lane+50), int(0.7*h1)), (255,0,0), 1, cv2.LINE_AA)
        
    #Scanning horizontally over the marker and detect high transition
    road_nomark=1
    #for k5 in range(-15,15):
    for k5 in range(-120+range_adjustment_left,120):
            
        value = img2[int(0.7*h1),int(x2_lane+k5)]
        ...
        if k5>-120+range_adjustment_left:
            ...
            if value>1.1*previous:
                ...
                for k6 in range(0,100):
                    ...
                    cv2.line(img6, (int(x2_lane_scan-50), int(0.7*h1-k6)), (int(x2_lane_scan+50), int(0.7*h1-k6)), (255,0,0), 1, cv2.LINE_AA)

                    count_scanned_lines=count_scanned_lines+1
                        
                    out_of_the_mark=1
                    #for k7 in range(-10,10):
                    for k7 in range(-120+range_adjustment_left,120):
        
                        value_scan = img2[int(0.7*h1-k6),int(x2_lane_scan+k7)]
                        #if k7>-10:
                ...
                for k6 in range(0,100):
                    ...
                    cv2.line(img6, (int(x2_lane_scan-50), int(0.7*h1+k6)), (int(x2_lane_scan+50), int(0.7*h1+k6)), (0,0,255), 1, cv2.LINE_AA)                  
                ...
                if capture_frameindex_for_speed==0 and count_scanned_lines+count_scanned_lines_reverse>=40:
                    white_mark_hit=1
                    ...
                    if frameindex_for_speed_previous!=0:
                        ...
                        time=(frameindex_for_speed-frameindex_for_speed_previous)/30
                        speed=(40/time)*0.682
                        speed_read_flag=1
                        ...
                        if count_scanned_lines+count_scanned_lines_reverse>40: #and count_scanned_lines>20: #meaning correct road mark
                            ...
                            #correction_factor=(count_scanned_lines_reverse/95)*10
                            #correction_factor=(count_scanned_lines_reverse/55)*10
                            ...
                            #correction_factor2=(count_scanned_lines_reverse_for_speed_previous/95)*10
                            #correction_factor2=(count_scanned_lines_reverse_for_speed_previous/55)*10
    ...
    return speed,road_nomark,capture_frameindex_for_speed,frameindex_for_speed,white_mark_hit,speed_read_flag,count_scanned_lines_reverse_for_speed,count_scanned_lines_for_speed,offset,offset_at_end_of_mark,offset_at_middle_of_mark
